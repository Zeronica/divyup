Meteor.methods({
	'setPickupLocation': function(data) {
		if (!data.pickup_id)
...
// 	} 
// 	else {	
// 		// else determine whether the currenttime is inside delivery window for the store
...
// 			arrival_time = Meteor.myFunctions.getDeliveryStartForDay(delivery_start, currentTime);
// 		}
// 	}
...
// 	if (!d)
// 		throw new Meteor.Error(422, 'the divy_id provided is invalid'+ data.divy_id);
...
// 	if (!divy_id)
// 		throw Meteor.Error(500, "error");
// 	deliver_time = (new Date()).getTime();
// 	Meteor.call('setKickstarterDivy', {store_id: data.store_id, divy_id: divy_id, deliver_time: deliver_time});
