Meteor.methods({
	'checkStatus': function() {
		...
		for (d in Divys.find({archived: false}).fetch()) {
			checkDivy(d);
		}
	}
});
...
// d as divy object
var checkDivy = function(d) {
	// get the total in divy right now;

	// if total in divy meets the quota, then create a processingdivy.
	// lock the divy,
	// delete all the pickupDivys
