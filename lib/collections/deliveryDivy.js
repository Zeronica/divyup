DeliveryDivy = new Meteor.Collection('deliveryDivy');
/*
divy_id
deliver_time
*/
...
var setDeliveryDivy = function(data) {
	if (!data.store_id && !data.user_id && data.user_id != Meteor.userId())
		...
		d = DeliveryDivy.find({sort: {'deliver_time': 1}, limit:1});
		...
		if (d.count() === 0) {
			...
			DeliveryDivy.insert({divy_id: divy_id, deliver_time: deliver_time});
			return divy_id;
		}
