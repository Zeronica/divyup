...
Meteor.methods({
	...
	'lockKickstarterDivy': function(data) {
		return lockKickstarterDivy(data);
	}
});
...
var lockKickstarterDivy = function(data) {
	if (!data._id)
		throw new Meteor.Error(422, 'please provide _id.');

	return KickstarterDivys.update({_id: data._id}, {locked: true});
}